...
export function MainNavbar() {
  ...
  const submitSearch = () => {
    const url = keyword ? `${ROUTES.SEARCH}?keyword=${encodeURIComponent(keyword)}` : ROUTES.SEARCH;
    router.push(url);
  };
  ...
  return (
    <div
      className={`border-border/70 bg-background/90 sticky top-0 z-40 border-b backdrop-blur-xl transition-transform duration-300 ${
        isHidden ? "-translate-y-full" : "translate-y-0"
      }`}
    >
      <div className="mx-auto flex h-20 w-full max-w-7xl items-center gap-3 px-4">
        <MobileNavDrawer items={navigationItems} />
        <Link href={ROUTES.HOME} className="text-foreground/90 font-semibold tracking-tight">
          SportBooking
        </Link>
        ...
        <div className="ml-auto flex items-center gap-2 md:ml-0">
          <Button asChild variant="outline" size="icon" className="h-11 w-11 rounded-xl md:hidden">
        ...
        {isAuthenticated && user ? (
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="outline" className="bg-card h-11 gap-2 rounded-xl">
