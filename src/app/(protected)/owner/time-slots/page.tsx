...
export default function OwnerTimeSlotsPage() {
  ...
  const rows = useMemo(
    () => (timeSlotsQuery.data?.items ?? []).filter((slot) => ownerCourtIds.has(slot.courtId)),
    [timeSlotsQuery.data?.items, ownerCourtIds],
  );
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Manage time slots" description="Create and maintain your venue time slots" />
      ...
      {timeSlotsQuery.isLoading || courtsQuery.isLoading || venuesQuery.isLoading ? (
            ...
            <TableBody>
              {rows.map((slot) => (
                <TableRow key={slot.id}>
                  <TableCell>{ownerCourts.find((court) => court.id === slot.courtId)?.name ?? slot.courtId.slice(0, 8)}</TableCell>
